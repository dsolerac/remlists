package com.remlists.list.write.infrastructure.repository;
...
@SpringBootTest
public class RemListRepositoryTest {

    private Logger LOG = LoggerFactory.getLogger(RemListRepositoryTest.class);


    @Autowired
    @Qualifier(listWriteRepositoryJPA)
    ...
    class PersisNewRemlistTest_ValidTests {
        ...
        void valid_findRemlistByName() {
            ...
            //Given
            RemListName xxx = new RemListName("xxx");
            RemList list = new RemList(new Id(UUID.randomUUID()), xxx);
            RemList persistedList = repo.save(list);

            //When
            Optional<RemList> found = repo.<RemListName>findByName(xxx);
